...
def create(
    ...
    if feature:
        boundary_gpd = boundary_gpd.loc[[feature]].reset_index()
    else:
        boundary_gpd = boundary_gpd.iloc[[0]].reset_index()

    if not mask and resolution:
    ...
    if align and not resolution:
        boundary_gpd = align_to_resolution(boundary_gpd, resolution)
    else:
