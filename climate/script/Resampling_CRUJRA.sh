...
indir='/Volumes/LaCie/DATA/CLIMATE/CRUJRA/data/'
## Path to outputs 
outdir='/Volumes/LaCie/DATA_processed/CLIMATE/CRUJRA_60N_EPSG6931/'
## List of climate variables
varlist=('dlwrf' 'dswrf' 'pre' 'pres' 'spfh' 'tmp')
...
### Resampling and reprojection: Process for a single sub-region
...
## Compute [1970-2000] averages
filelist=()
for y in {1970..2000}; do
	print $y
	filelist+=($outdir$dirname'/crujra.v2.4.5d.'$y'r.nc')
done
echo ${filelist[*]}
ncea -O -h ${filelist[*]} $outdir$dirname'/crujra.v2.4.5d.1970_2000r.nc'


### Resampling and reprojection: Process for a multiple sub-regions
## Number of sections on the horizontal (nx) and vertical (ny) sides to split the main mask into sub-regions
nx=10
ny=10
